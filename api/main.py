...
def calculate_days_on_market(listings: List[CarListing], min_listings: int = 3) -> List[Dict]:
    ...
    The returned days measure how long a listing has been observed by this app;
    they are never represented as confirmed time-to-sale.
    """
...
def analyze_price_ranges(listings: List[CarListing]) -> Dict[str, Dict]:
    """Group verified asking-price observations by price range."""
    price_ranges = {
...
def create_scatter_data(listings: List[CarListing]) -> List[Dict]:
    """Create asking-price scatter plot data using days tracked by this app."""
    scatter_data = []
...
async def get_dashboard_data(
    ...
    try:
        ...
        # Identify recently observed models without inferring that they were sold.
